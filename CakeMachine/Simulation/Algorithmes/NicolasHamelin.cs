...
{
    internal class NicolasHamelin : Algorithme
    {
        /// <inheritdoc />
        ...
        private class OrdreProduction
        {
            ...
            public async IAsyncEnumerable<GâteauEmballé> ProduireAsync()
            {
                while (!_token.IsCancellationRequested)
                {
                    ...
                    {
                        Parallel.ForEach(tâchesEmballage, i =>
                        {
                            gâteauxCuits = ProduireEtCuireParBains(_usine.OrganisationUsine.ParamètresCuisson.NombrePlaces, 6);
                        });
                ...
                ushort nombreBains)
            {
                ...
                {
                    Parallel.ForEach(bainGâteauxCuits, i =>
                    {
                        gâteauxCrus = PréparerConformesParBainAsync(nombrePlacesParFour, nombreBains);
                    });
                ...
                ushort gâteauxParBain, ushort bains)
            {
                ...
                for (var i = 0; i < totalAPréparer; i++)
                {
                    ...
                    if (buffer.Count != gâteauxParBain)
                    {
                        continue;
                    }
